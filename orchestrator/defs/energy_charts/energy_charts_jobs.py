...
daily_partitioned_price_assets = []
daily_partitioned_price_ops_config = {}
daily_partitioned_price_keys = ["price"]

for key in daily_partitioned_price_keys:
    asset_key = AssetKey(['EXTRACTION', f'energy_charts_to_duckdb_{key}'])
    daily_partitioned_price_assets.append(asset_key)
    op_key = f"energy_charts_ingestion_{key}"
    daily_partitioned_price_ops_config[op_key] = {
...
energy_charts_daily_partitioned_import_job = define_asset_job(
    ...
    selection=daily_partitioned_price_assets,
    config={
        "ops": daily_partitioned_price_ops_config
    },
